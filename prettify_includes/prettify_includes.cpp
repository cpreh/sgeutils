...
try
{
	...
	fcppt::optional::maybe(
		...
		[
			...
			fcppt::io::cerr()
				<<
				FCPPT_TEXT("Failed to execute command (no result) ")
				<<
				_command
				<<
				FCPPT_TEXT('\n');
		...
		)
		{
			...
			)
			{
				fcppt::io::cerr()
					<<
					FCPPT_TEXT("Failed to execute command ")
					<<
					_command
					<<
					FCPPT_TEXT('\n');
...
)
{
	...
	fcppt::string const command_line{
	...
	)
	{
		fcppt::either::match(
			...
			[
				...
				fcppt::filesystem::recursive_directory_range const &_range
			)
			{
				for(
					std::filesystem::path const &inner
					:
					_range
				)
				{
